...
def sample_from_planes(plane_axes, plane_features, coordinates, mode='bilinear', padding_mode='zeros', box_warp=None):
    ...
    ## box_warp: the covered area of tri-plane/volume
    ## so the sampled coordinated will be normalized to the tri-plane/volume here, no need to process it like in the get_rays_ndc in the mvsnerf
...
def sample_from_volume(volume_features, coordinates, mode='bilinear', padding_mode='zeros', box_warp=None):
    
    #### MVSNerf gird sample as below:
    # input_feat = index_point_feature_batch(volume_feature, rays_ndc, grid_sample_mode=grid_sample_mode) if torch.is_tensor(volume_feature) else volume_feature(rays_ndc)
    # features = F.grid_sample(volume_feature, grid, align_corners=False, mode=grid_sample_mode)[:,:,0].permute(0,2,3,1) # mode='nearest'
    
    assert padding_mode == 'zeros'
    N, C, D, H, W = volume_features.shape
    # st()
    _, M, _ = coordinates.shape # N,M,3 where N is batch size
    n_planes = 1
    # plane_features = plane_features.view(N*n_planes, C, H, W)

    ## box_warp: the covered area of tri-plane/volume
    ## so the sampled coordinated will be normalized to the tri-plane/volume here, no need to process it like in the get_rays_ndc in the mvsnerf
    coordinates = (2/box_warp) * coordinates # -1~1
    coordinates = coordinates[:, None, None, ...] # shape: (B,1,1,M,3) to correctly use 3D grid sampled

    # projected_coordinates = project_onto_planes(plane_axes, coordinates).unsqueeze(1)
    output_features = torch.nn.functional.grid_sample(volume_features, coordinates.float(), mode=mode, padding_mode=padding_mode, align_corners=False)[:,:,0] #.permute(0, 2, 3, 1).reshape(N, n_planes, M, C)
    output_features = output_features.permute(0, 2, 3, 1).reshape(N, n_planes, M, C)
    return output_features
