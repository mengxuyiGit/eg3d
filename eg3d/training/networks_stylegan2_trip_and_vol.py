...
# directly modify this to generate volume
@persistence.persistent_class
class SynthesisNetwork(torch.nn.Module):
    ...
    def voxelize_spconv_sparse_pointnet(self, pc, grid_size=[], 
        ...
        batch_bbox = torch.tensor([
            [-box_warp/2, -box_warp/2, -box_warp/2],
            [box_warp/2, box_warp/2, box_warp/2]
        ...
        ## direct batch voxelization
        
        batch_voxel_size = (batch_bbox[:,:,1:]-batch_bbox[:,:,:1])/grid_size
        ...
        if pointnet_input== 'local_xyz':
            ...
            for i_batch in range(len(batch_xyz_cube_pos)):
                for i_view in range(len(batch_xyz_cube_pos[i_batch])):
                    cat_pt_fea.append(batch_pcl_local[i_batch, i_view])
                    cat_pt_ind.append(F.pad(batch_xyz_cube_pos[i_batch, i_view],(1,0),'constant',value = i_batch*V+i_view))
            cat_pt_fea = torch.cat(cat_pt_fea,dim=0)
            cat_pt_ind = torch.cat(cat_pt_ind,dim = 0)
            # st()
        else:
            raise NotImplemented(False)
...
class Generator(torch.nn.Module):
    def __init__(self,
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetwork(w_dim=w_dim, volume_res=self.volume_res, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
