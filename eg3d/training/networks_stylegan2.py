...
class SynthesisLayer(torch.nn.Module):
    ...
    def forward(self, x, w, noise_mode='random', fused_modconv=True, gain=1):
        ...
        # st()
        assert noise_mode == 'none'
        in_resolution = self.resolution // self.up
        misc.assert_shape(x, [None, self.in_channels, in_resolution, in_resolution])
        
        if w is not None:
            styles = self.affine(w)
            # print("style shape:", styles.shape, "x shape", x.shape)
            # exit()
        else:
            styles = torch.ones([x.shape[0], x.shape[1]]).to(x.device)
...
class ToRGBLayer(torch.nn.Module):
    ...
    def forward(self, x, w, fused_modconv=True):
        if w is not None:
            ...
            styles = torch.ones([x.shape[0], x.shape[1]]).to(x.device) * self.weight_gain
...
class SynthesisBlock(torch.nn.Module):
    ...
    def forward(self, x, img, ws, force_fp32=False, fused_modconv=None, update_emas=False, **layer_kwargs):
        ...
        # Input.
        # print("x.shape", x.shape, 'ws.shape', ws.shape)
