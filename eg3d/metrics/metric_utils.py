...
def compute_feature_stats_for_dataset(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, data_loader_kwargs=None, max_items=None, **stats_kwargs):
    ...
    for images, _labels, _pc, _proj in torch.utils.data.DataLoader(dataset=dataset, sampler=item_subset, batch_size=batch_size, **data_loader_kwargs):
        if images.shape[1] == 1:
    ...
    while not stats.is_full():
        ...
        if isinstance(opts.G, VolumeGenerator): # c_iter generate both c and pc
            for _i in range(batch_size // batch_gen):
                ...
                c, pc = next(c_iter)
                print('compute_feature_stats_for_generator', _i)
